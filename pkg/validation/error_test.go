...
func Test_IsAppConfigMapNotFound(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "case 1: wrong type of error",
			errorMessage:  "admission webhook \"apps.app-admission-controller-unique.giantswarm.io\" denied the request: kube config not found error: kubeconfig secret `u9q0r-cluster-values` in namespace `u9q0r` not found",
			expectedMatch: false,
		},
	}
